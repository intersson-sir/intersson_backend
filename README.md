...
### Статистика (dashboard): таймаут и обработка ошибок

Рекомендации для фронта при запросах к `/api/stats/dashboard/` (и к `/api/stats/platform/:platform/`):

1. **Таймаут запроса (15 секунд)**  
   Если ответ не приходит за 15 секунд (бэкенд не поднят, сеть и т.п.), запрос должен прерываться по таймауту, а не висеть бесконечно.

2. **Обработка ошибок**  
   В состоянии сохранять текст ошибки (в т.ч. «Request timed out»). Вместо бесконечной загрузки показывать сообщение об ошибке и кнопку **Try again**.

3. **Кнопка «Try again»**  
   По нажатию снова вызывать загрузку статистики для текущего выбранного периода.

Итог: при недоступном бэкенде через 15 секунд пользователь видит сообщение об ошибке и кнопку повтора, а не бесконечный спиннер.

**Что проверить у себя:**

- **Мок без бэкенда:** в `.env.local` задать `NEXT_PUBLIC_USE_MOCK=true` — дашборд будет брать данные из мока и не обращаться к API.
- **С бэкендом:** убедиться, что бэкенд запущен и по адресу `NEXT_PUBLIC_API_URL` (по умолчанию `http://localhost:8000`) доступен эндпоинт `GET /api/stats/dashboard/?period=1d|7d|30d` и что в запрос подставляется корректный токен (после логина через приложение).
